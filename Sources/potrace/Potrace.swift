...
final class Potrace {
    ...
    */
    ...
    func getBezierPath(scale size: Double = 1.0) -> CGMutablePath {

        // func path(curve: Curve, bezierPath: UIBezierPath) {
        func path(curve: Curve, bezierPath: CGMutablePath) {
                
            func bezier(i: Int) {

                bezierPath.addCurve(
                    to: CGPoint(x: (curve.c[i * 3 + 2].x * size), y: (curve.c[i * 3 + 2].y * size)),
                    control1: CGPoint(x: (curve.c[i * 3 + 0].x * size), y: (curve.c[i * 3 + 0].y * size)),
                    control2: CGPoint(x: (curve.c[i * 3 + 1].x * size), y: (curve.c[i * 3 + 1].y * size))
                )
            }
